...
async def handle_detailed_calculation(callback: CallbackQuery, state: FSMContext):
    ...
    message_text = (
        ...
        f"ℹ️ Закупочная стоимость рассчитана приблизительно, данные по весу, объёму и цене взяты из карточки товара и могут отличаться от фактических параметров, можете скорректировать данные по кнопкам ниже."
    )
