...
class CalculationWorker:
    ...
    async def _process_express_calculation(self, calculation_id: str, data: dict):
        ...
        try:
            ...
            # Get product data for TN VED selection
            ...
            product_volume = self.wb_parser.get_product_volume(product_with_filled_fields)
            
            # Request TN VED code from GPT
    ...
    def _format_detailed_result(self, detailed_result: dict, product_data: dict, tnved_data: dict = None) -> str:
        ...
        if white.get("ok"):
            ...
            if tn_ved_code:
                tn_ved_line = f"код ТНВЭД: {tn_ved_code}"
                if duty_rate_display and vat_rate_display:
                    tn_ved_line += f" ({duty_rate_display} пошлина, {vat_rate_display} НДС)"
                elif duty_rate_display:
                    tn_ved_line += f" ({duty_rate_display} пошлина)"
            ...
            message_parts.extend([
                ...
                f"Документы: {white.get('docs_rub', 0):.2f} RUB",
                f"Брокер: {white.get('broker_rub', 0):.2f} RUB",
                f"Пошлина: {white.get('duty_rub', 0):.2f} RUB{f' ({duty_rate_display})' if duty_rate_display else ''}",
