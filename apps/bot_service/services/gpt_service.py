...
{{
    ...
    async def _get_tn_ved_code_with_card_data(
        ...
        prompt_stage1 = f"""Подбери код ТН ВЭД для товара. Используй поиск в интернете для поиска информации на сайте ifcg.ru.

ВАЖНО: Отвечай кратко, только JSON без дополнительных рассуждений.

{basic_info}

Используй web search для поиска кода ТН ВЭД на сайте ifcg.ru.
...
Верни только текст сообщения в указанном формате без дополнительных комментариев и без markdown блоков."""
    ...
    async def _call_gpt_api(self, prompt: str, model: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        # Search models (gpt-4o-search-preview, gpt-5-search-api) don't support temperature and response_format
        if "gpt-5" in model_to_use:
            ...
            # Search models don't support temperature and response_format parameters
            # Rely on prompt instructions to get JSON response
        else:
            payload["max_tokens"] = 200
