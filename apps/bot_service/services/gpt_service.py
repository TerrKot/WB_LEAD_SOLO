...
3. Включает детальную причину из блока выше, адаптированную под клиентский язык (без технических терминов, понятно для бизнеса)
...
Ответь только текстом сообщения в формате HTML, без дополнительных комментариев."""
        ...
        try:
            ...
            if not response:
                logger.error(
                    "gpt_forbidden_category_message_no_response",
                    product_name=product_name,
                    category=category
                )
                return None
            ...
            # Если да, извлекаем только текст сообщения
            if content.startswith("{") and content.endswith("}"):
                try:
                    ...
                    if isinstance(json_data, dict) and "message" in json_data:
                        ...
                        logger.info(
                            "gpt_forbidden_category_message_json_parsed",
                            product_name=product_name,
                            category=category
                        )
            ...
            logger.error(
                "gpt_forbidden_category_message_json_error",
                product_name=product_name,
                category=category,
                error=str(e)
            )
...
В секции "Про объём:":
Чем больше объём партии, тем выгоднее будет использование белой логистики, при решении всех вопросов с документами и подготовкой к транспортировке.
Обязательно включи информацию о рекомендуемом выгодном объеме из контекста (если она есть), используя точные значения веса, объема и количества штук без изменений.
