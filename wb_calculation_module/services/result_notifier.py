...
async def send_notification(
    ...
    """
    Send notification to notification group in format: @username | Статус: 🟢 | WB/Ozon: артикул | ТН ВЭД: код (ссылка)
    
    Args:
        bot: Telegram Bot instance
        username: Telegram username (without @)
        status: Status emoji (🟢, 🟡, 🟠, 🔴, ⚪️)
        article_id: Article ID (WB or Ozon)
        tn_ved_code: TN VED code (optional)
        marketplace: Marketplace type ("ozon" or "wildberries", default: "wildberries")
        ozon_url: Ozon product URL (required if marketplace is "ozon")
    """
    ...
    try:
        ...
        # Format article link - ensure article_id is properly converted to string and validated
        try:
        ...
        # Format link based on marketplace
        if is_ozon:
            # For Ozon, use provided URL or construct from article_id
            if ozon_url:
                product_url = ozon_url
            else:
                # Fallback: construct Ozon URL (may not work if slug is missing)
                product_url = f"https://www.ozon.ru/product/{article_id_str}/"
            marketplace_label = "Ozon"
        else:
            # For Wildberries
            product_url = f"https://www.wildberries.ru/catalog/{article_id_str}/detail.aspx"
            marketplace_label = "WB"
        
        article_link = f"<a href=\"{product_url}\">{article_id_str}</a>"
        
        # Format notification message
        notification_text = f"{username_str} | Статус: {status} | {marketplace_label}: {article_link}"
...
class ResultNotifier:
    ...
    def _get_marketplace_info(self, result: Dict[str, Any]) -> tuple[Optional[str], Optional[str]]:
        """
        Extract marketplace type and Ozon URL from result.
        
        Args:
            result: Calculation result dictionary
            
        Returns:
            Tuple of (marketplace, ozon_url):
            - marketplace: "ozon" or "wildberries" or None
            - ozon_url: Ozon product URL if marketplace is "ozon", None otherwise
        """
        # Check result directly
        marketplace = result.get("marketplace")
        if marketplace == "ozon":
            # Try to get Ozon URL from various sources
            ozon_url = (
                result.get("ozon_url") or
                result.get("input_data", {}).get("url") or
                result.get("product_data", {}).get("ozon_url")
            )
            logger.info(
                "marketplace_info_found_direct",
                marketplace=marketplace,
                ozon_url=ozon_url,
                has_ozon_url_in_result="ozon_url" in result,
                has_input_data="input_data" in result,
                has_product_data="product_data" in result
            )
            return ("ozon", ozon_url)
        
        # Check product_data for Ozon indicators
        product_data = result.get("product_data")
        if product_data:
            if product_data.get("ozon_url") or product_data.get("ozon_product_id"):
                ozon_url = product_data.get("ozon_url") or result.get("input_data", {}).get("url")
                logger.info(
                    "marketplace_info_found_in_product_data",
                    marketplace="ozon",
                    ozon_url=ozon_url,
                    has_ozon_url="ozon_url" in product_data,
                    has_ozon_product_id="ozon_product_id" in product_data
                )
                return ("ozon", ozon_url)
        ...
        if input_data:
            ...
            if url and "ozon.ru" in url.lower():
                logger.info(
                    "marketplace_info_found_in_input_data",
                    marketplace="ozon",
                    ozon_url=url
                )
                return ("ozon", url)
        
        # Default to Wildberries
        logger.info(
            "marketplace_info_default_wb",
            result_keys=list(result.keys()),
            has_marketplace="marketplace" in result,
            marketplace_value=result.get("marketplace"),
            has_product_data=bool(product_data),
            has_input_data=bool(input_data)
        )
    ...
    async def _send_result_message(self, user_id: int, status_message_id: int, result: Dict[str, Any]):
        ...
        if calculation_type == "detailed":
            ...
            if article_id:
                ...
                marketplace, ozon_url = self._get_marketplace_info(result)
                try:
                    await send_notification(self.bot, username, "🔴", article_id, tn_ved_code, marketplace=marketplace, ozon_url=ozon_url)
                except Exception as e:
            ...
            if article_id:
                ...
                logger.info(
                    ...
                    ozon_url=ozon_url,
                    result_keys=list(result.keys()) if result else [],
                    has_marketplace="marketplace" in result if result else False,
                    has_ozon_url="ozon_url" in result if result else False,
                    has_input_data="input_data" in result if result else False,
                    has_product_data="product_data" in result if result else False
                )
                try:
            ...
            if article_id:
                ...
                try:
                    await send_notification(self.bot, username, "⚪️", article_id, tn_ved_code, marketplace=marketplace, ozon_url=ozon_url)
                    logger.info(
