...
4. После успешной инициализации запустит `bot_service` и **10 воркеров** (worker)

**Количество воркеров:** По умолчанию запускается 10 воркеров. Можно изменить через переменную окружения:
```bash
...
```bash
docker-compose up -d
docker-compose up -d --scale worker=10  # Для 10 воркеров
```
...
- ✅ Автоматическое восстановление количества воркеров (если упал один из 10)
